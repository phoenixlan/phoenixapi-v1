...
@validate(json_body={'firstname': str, 'surname': str, 'password': str, 'passwordRepeat': str, 'email': str, 'emailRepeat': str, 'gender': str, "dateOfBirth": str, 'phone': str, 'guardianPhone': str, 'event_notice_consent': bool})
def register_user(context, request):
    ...
    if existingEmail is not None or existingPhone is not None:
        request.response.status = 400
        return {
            "error": "A user by this phone number or e-mail already exists"
        }
    ...
    user = User(email, request.json_body["password"], firstname, surname, birthdate, gender, request.json_body["phone"])
